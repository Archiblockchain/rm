...
class AssetTransfer extends Contract {
    ...
    async CreateAsset(ctx, id, newasset) {
        console.info(`Asset ${JSON.stringify(newasset)} initialized`);
        ctx.stub.putState(id, Buffer.from(JSON.stringify(newasset)));
        return JSON.stringify(newasset);
    }

    async CreateAssetOption2(ctx, id) {
        const newasset = 	{
            RM_Application_No: 'RM0004',
            Primary_Residence_Property_Information: {
            Subject_Property_Address: {
                street: '301 Arcadia Drive',
                city: 'Bloomington',
